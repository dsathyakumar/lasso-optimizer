            ...
            Object.keys(bindings).forEach((param, idx) => {
                ...
                if (traversalPath.scope.hasOwnBinding(param)
                    ...
                    if (paramBinding.referenced) {
                        ...
                        // many exportables may exist for a given module.
                        // so get rid of it at the end
                        /* if (idx === 1) {
                            const objExp = types.objectExpression(exportables);
                            const ret = types.returnStatement(objExp);
                            traversalPath.get('body').node.body.push(ret);
                            // remove the binding & update the scope
                            traversalPath.scope.removeOwnBinding(param);
                            traversalPath.node.params = popParam(param, traversalPath.node.params);
                        } else if (idx === 0) {
                            // many require() may exist for a given module.
                            // so get rid of it at the end
                            traversalPath.scope.removeOwnBinding(param);
                            traversalPath.node.params = popParam(param, traversalPath.node.params);
                        }  */
